    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
                ...
                if ui.button("Generate Post-Quantum Identity...").clicked() {
                    ...
                    if let Some(priv_path) = file_dialog.save_file() {
                        ...
                        match priv_key.save_to_file(&priv_path) {
                            Ok(_) => {
                                match pub_key.save_to_file(&pub_path) {
                                    Ok(_) => self.status = format!("Identity generated!\nPrivate Key: {}\nPublic Key: {}", priv_path.display(), pub_path.display()),
            ...
            if self.auth_mode == AuthMode::Password {
                ...
                ui.horizontal(|ui| {
                    if ui.button(if self.mode_encrypt { "Select Public Key (.lvau-pub)" } else { "Select Private Key (.lvau-key)" }).clicked() {
            ...
            if self.mode_encrypt {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.radio_value(&mut self.profile, SecurityProfile::Paranoid, "Paranoid (2-Layer)");
                    ui.radio_value(&mut self.profile, SecurityProfile::Extreme, "Extreme (3-Layer Custom)");
                });
            ...
            if ui.add_enabled(can_proceed, egui::Button::new(if self.mode_encrypt { "Encrypt & Save" } else { "Decrypt & Save" })).clicked() {
                    ...
                    if let Some(out_path) = file_dialog.save_file() {
                        ...
                        let result = match self.auth_mode {
                            AuthMode::Password => {
                                ...
                                if self.mode_encrypt {
                                    encrypt_file_password(in_file, &temp_out, pwd, seed_val, self.profile.clone())
                                } else {
                            ...
                            AuthMode::KeyFile => {
                                ...
                                if self.mode_encrypt {
                                    if let Ok(pub_key) = HybridPublicKey::load_from_file(kp) {
                                        encrypt_file_keypair(in_file, &temp_out, &pub_key, self.profile.clone())
                                    } else {
                        ...
                        match result {
                            Ok(_) => {
                                if self.mode_encrypt && self.sfx {
                                    ...
                                    if !stub_path.exists() {
                                        self.status = format!("Error: lvau-stub.exe not found in {}", exe_dir.display());
                                        std::fs::remove_file(&temp_out).ok();
                                    } else {
                                        if let Err(e) = std::fs::copy(&stub_path, &out_path) {
                                            self.status = format!("SFX Copy Error: {:?}", e);
                                        } else {
                                            let mut out_f = std::fs::OpenOptions::new().append(true).open(&out_path).unwrap();
                                            ...
                                            self.status = format!("Success: SFX Output saved to {}", out_path.display());
                                        }
                                        std::fs::remove_file(&temp_out).ok();
                                    }
                                } else {
                                    self.status = format!("Success: Output saved to {}", out_path.display());
                                }
                            }
                            Err(e) => {
            ...
            if !self.status.is_empty() {
                ui.label(egui::RichText::new(&self.status).color(if self.status.starts_with("Error") { egui::Color32::RED } else { egui::Color32::GREEN }));
            }
